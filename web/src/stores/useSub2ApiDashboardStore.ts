...
refresh: (opts?: { hours?: number; background?: boolean }) => Promise<void>
 ...
 loadEvents: (params?: { page?: number; limit?: number; hours?: number }) => Promise<void>
...
refresh: async (opts) => {
    ...
    // Fire events refresh independently so it doesn't block range-dependent cards.
     void get().loadEvents({ page: 1, limit: 100, hours })
  } catch (error) {
     if (get()._refreshVersion !== version) return
...
loadRankings: async (dimension = 'user') => {
 ...
 loadEvents: async (params = {}) => {
    ...
    try {
      const hours = params.hours ?? get().currentHours
      const { page, limit } = params
      const events = await fetchSub2ApiEvents({ page, limit, hours })
      set({ events })
    } finally {
